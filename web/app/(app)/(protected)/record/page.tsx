...
export default function RecordLine() {
  ...
  const [isContributed, setIsContributed] = useLocalStorage<boolean>(
    "isContributed",
    false,
  );
  ...
  const savePGN = async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/variations/${code ? `contribute/${code}` : "record"}`,
      {
        credentials: "include",
        method: "POST",
        headers: {
    ...
    <div className="flex flex-col lg:flex-row bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-gray-100 relative">
      ...
      <aside className="w-full lg:w-[430px] bg-gray-800/95 p-8 flex flex-col gap-5 shadow-xl rounded-t-3xl lg:rounded-l-3xl">
        ...
        <div className="bg-gray-700 p-4 rounded-xl  shadow-inner">
          ...
          <div className="h-2 bg-gray-600 rounded mt-2">
            <div
              ...
              style={{
                width: `${((currentMoveIndex / (moveHistory?.length || 1)) * 100).toFixed(1)}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col w-full gap-2">
          <Link
            href={`/lessons/recorded-lines`}
            className="flex flex-col gap-4 w-full"
          >
            <Button text="Play Recorded Lines" icon="→" />
          </Link>
          {isContributed && (
            <Link href={`/lessons/${code ? code : "recorded-lines"}`}>
              <Button
                text="Visit Latest Contribution"
                icon="→"
                onClick={() => {

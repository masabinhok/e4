...
const Profile = () => {
  ...
  const stats: Stat = {
    ...
    recentAchievements: [
      { name: "Opening Master", description: "Learned 50 opening lines", icon: BookOpen },
      { name: "Streak Champion", description: "15-day study streak", icon: Zap },
      { name: "Tactician", description: "Solved 100 puzzles", icon: Target }
    ]
  }

  const learningProgress: ProgressItem[] = [
    { category: 'Recorded Lines', count: user?.recordedLines?.length || 0, color: 'bg-blue-500', icon: BookOpen },
    { category: 'Custom Lines', count: user?.customLines?.length || 0, color: 'bg-purple-500', icon: Edit3 },
    { category: 'Practiced Lines', count: user?.practicedLines?.length || 0, color: 'bg-green-500', icon: Target },
    { category: 'Quizzed Lines', count: user?.quizzedLines?.length || 0, color: 'bg-orange-500', icon: Brain },
    { category: 'Learned Lines', count: user?.learnedLines?.length || 0, color: 'bg-emerald-500', icon: Trophy }
  ]
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="text-gray-400 text-sm">{count} items</div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex space-x-1 bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-1 mb-8">
          {['overview', 'progress', 'achievements'].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab as 'overview' | 'progress' | 'achievements')}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <StatCard title="Total Games" value={stats.totalGames} subtitle="This month: +23" icon={Target} color="text-blue-400" />
              <StatCard title="Best Rating" value={stats.bestRating} subtitle="All-time high" icon={Trophy} color="text-yellow-400" />
              <StatCard title="Study Hours" value={stats.totalStudyTime} subtitle="This month: 28h" icon={Clock} color="text-green-400" />
              <StatCard title="Win Rate" value={`${stats.winRate}%`} subtitle="Last 50 games" icon={TrendingUp} color="text-purple-400" />
            </div>
        ...
        {activeTab === 'progress' && (
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
            <h2 className="text-2xl font-bold text-white mb-6">Detailed Progress</h2>
              ...
              {learningProgress.map((item, index) => (
                <div key={index} className="bg-white/5 rounded-xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400">Total: {item.count} items</p>
                      </div>
                    </div>
                    <div className="text-right">
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div className={`h-2 rounded-full ${item.color}`} style={{ width: `${Math.min((item.count / 20) * 100, 100)}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {stats.recentAchievements.map((achievement, index) => (
                <div key={index} className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 border border-yellow-500/20 rounded-xl p-6 hover:scale-105 transition-all duration-300">

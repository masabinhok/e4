...
const Dashboard = ({ users, openings, variations }: DashboardProps) => {
  const recordingsCount = variations.filter(
    (variation) => variation.code === "recorded-pgns",
  ).length;
  const customsCount = variations.filter(
    (variation) => variation.code === "custom-pgns",
  ).length;
  const contributionsCount = variations.filter(
    (variation) => variation.contributor,
  ).length;
            ...
            {users.slice(0, 4).map((user) => (
              ...
              >
                ...
                <div className="text-right">
                  <p
                    className={`font-medium ${user.role === Role.Admin ? "text-green-400" : "text-blue-400"}`}
                  >
                    {user.role}
                  </p>
                </div>
